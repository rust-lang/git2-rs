...
pub fn message_prettify<T: IntoCString>(message: T, comment_char: Option<u8>) -> Result<String, Error> {
    let ret = Buf::new();
    let message = try!(message.into_c_string());
    unsafe {
        try_call!(raw::git_message_prettify(ret.raw(), message, comment_char.is_some() as c_int, comment_char.unwrap_or(0) as c_char));
    }
